...
void Operation::addseg(const QString msg) {
    ...
    if (segs.size() == 2) {
        ...
        if (it != segments.seg.end()) {
            ...
            for (int i = 0; i < it->row.size(); i++) {
                double dist = distance(segs[0].row[comparedIndex].x, it->row[i].x, segs[0].row[comparedIndex].y,
                                       it->row[i].y, segs[0].row[comparedIndex].z, it->row[i].z);
                if (dist < mindist) {
                    mindist = dist;
                    index = i;
                }
            ...
            if (index == -1) {
                ...
                qDebug() << segs[0].row[comparedIndex].x << " " << segs[0].row[comparedIndex].y << " " << segs[0].row[
                    comparedIndex].z;
                it->printInfo();
            }
            else {
    ...
    if (segs.size() == 3) {
        ...
        if (it != segments.seg.end()) {
            ...
            double mindist = 5;
            for (int i = 0; i < it->row.size(); i++) {
                double dist = distance(segs[0].row[segs[0].row.size() - 1].x, it->row[i].x,
            ...
            if (index == -1) {
                qDebug() << "INFO:cannot find nearest point in first connected seg";
                qDebug() << segs[0].row[segs[0].row.size() - 1].x << " " << segs[0].row[segs[0].row.size() - 1].y << " "
                        << segs[0].row[segs[0].row.size() - 1].z;
                it->printInfo();
            }
            else {
        ...
        if (it != segments.seg.end()) {
            ...
            double mindist = 5;
            for (int i = 0; i < it->row.size(); i++) {
                double dist = distance(segs[0].row[0].x, it->row[i].x, segs[0].row[0].y, it->row[i].y, segs[0].row[0].z,
                                       it->row[i].z);
                if (dist < mindist) {
                    mindist = dist;
                    index = i;
...
void Operation::splitseg(const QString msg) {
    ...
    for (int i = 1; i < segs.size(); i++) {
        ...
        if (distance(segs[i].row[segs[i].row.size() - 1].x, point2.x, segs[i].row[segs[i].row.size() - 1].y, point2.y,
                     segs[i].row[segs[i].row.size() - 1].z, point2.z) < 0.3) {
            segs[i].row[segs[i].row.size() - 1].x = point2.x;
            segs[i].row[segs[i].row.size() - 1].y = point2.y;
            segs[i].row[segs[i].row.size() - 1].z = point2.z;
        }
        if (distance(segs[i].row[segs[i].row.size() - 1].x, point1.x, segs[i].row[segs[i].row.size() - 1].y, point1.y,
                     segs[i].row[segs[i].row.size() - 1].z, point1.z) < 0.3) {
...
void Operation::addmarkers(const QString msg) {
    ...
    for (int i = 0; i < errorMarkers.size(); i++) {
        QString curMarker = QString("%1 %2 %3 %4 %5 %6").arg(errorMarkers[i].color.r).arg(errorMarkers[i].color.g).
                arg(errorMarkers[i].color.b).arg(errorMarkers[i].x).arg(errorMarkers[i].y).arg(errorMarkers[i].z);
...
void Operation::simpleConnectExecutor(V_NeuronSWC_list&segments, vector<segInfoUnit>&segInfo) {
    ...
    if ((segInfo.at(0).head_tail == -1 || segInfo.at(0).head_tail == 2) && (
            segInfo.at(1).head_tail == -1 || segInfo.at(1).head_tail == 2)) {
        flag = true;
        if (segInfo.at(0).nodeCount >= segInfo.at(1).nodeCount) {
            ...
            qDebug() << "secondary seg length:" << branchSeg.nodeCount << "   secondary seg orient:" << branchSeg.
                    head_tail;
        }
        else {
    ...
    if ((segInfo.at(0).head_tail != -1 && segInfo.at(0).head_tail != 2) ^ (
        ...
        if (branchSeg.head_tail == 2) // branch to tail
        {
            ...
            segments.seg[mainSeg.segID].row.insert(segments.seg[mainSeg.segID].row.end(),
                                                   segments.seg[branchSeg.segID].row.begin(),
                                                   segments.seg[branchSeg.segID].row.end());
            ...
            (segments.seg[mainSeg.segID].row.end() - 1)->parent = (
                segments.seg[mainSeg.segID].row.begin() + ptrdiff_t(mainSeg.head_tail - 2))->n;
            segments.seg[branchSeg.segID].to_be_deleted = true;
            segments.seg[branchSeg.segID].on = false;
        }
